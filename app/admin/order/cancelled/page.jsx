...
export default function OrdersPage() {
  ...
  // ✅ Fetch orders API
  const fetchOrders = async (status = "cancelled") => {
    try {
      setLoading(true);
...
const rows = orders
  .filter(order => order.orderStatus === "cancelled") // 🔥 only placed orders
  .map(order => ({
    id: order._id,
    fullId: order._id,
  ...
  return (
    <div className="px-4">
      <h1 className="text-xl font-semibold mb-4">Cancelled Orders</h1>
      <Box sx={{ width: "100%" }}>
        <DataGrid
          rows={rows}
          columns={columns}
